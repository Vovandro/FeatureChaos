...
(function (global) {
  ...
  Client.prototype.isEnabled = function(featureName, seed, attrs){ var cfg=this.features.get(featureName); if(!cfg) return false; var percent=-1, hashSeed=seed, keyLevel=null; if(attrs){ for (var k in attrs){ var v=attrs[k]; var kc=cfg.keys[k]; if(!kc) continue; if (kc.items[v]!==undefined){ percent = kc.items[v]; hashSeed=String(v); break; } if (keyLevel===null) keyLevel = kc.all; } } if (percent<0 && keyLevel!==null){ percent=keyLevel; hashSeed=seed; } if (percent<0){ percent=cfg.all; hashSeed=seed; } percent=clampPercent(percent); var enabled=bucketHit(featureName, hashSeed, percent); if (enabled && this.autoStats) this.track(featureName); return enabled; };
